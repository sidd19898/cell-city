...
addtocartbtnDom.forEach((addtocartbtnDom) => {
  addtocartbtnDom.addEventListener("click", () => {
    ...
    const IsinCart =
      cart.filter((cartItem) => cartItem.name === product.name).length > 6;
    if (IsinCart === false) {
      cartDom.insertAdjacentHTML(
        "beforeend",
        `
  <div class="d-flex flex-Row shadow-sm card cart-items mt-2 mb-3 animated flipInX Ccontainer">
    <div class="p-2 cart-img">
        <img src="${product.img}" alt="${product.name}" style="max-width: 70px;"/>
    </div>
    <div class="p-2 mt-3 cart-name">
        <p class="text-info cart_item_name">${product.name}</p>
    </div>
    <div class="p-2 mt-3 cart-price">
        <p class="text-success cart_item_price">${product.price}</p>
    </div>
    <div class="p-2 mt-3 ml-auto">
        <button class="btn badge badge-secondary plus-minus-cross-btn" type="button" data-action="increase-item">&plus;
    </div>
    <div class="p-2 mt-3">
      <button class="btn badge badge-info plus-minus-cross-btn" type="button" data-action="decrease-item">&minus;
    </div>
    <div class="p-2 mt-3">
      <button class="btn badge badge-danger plus-minus-cross-btn" type="button" data-action="remove-item" onclick="pRemove()">&times;
    </div>
    <div class="p-2 mt-3">
      <p class="text-success cart_item_quantity cart-quantity">${product.quantity}</p>
    </div>
    <script src="/js/cart-no.js"></script>
  </div> `
      );
      ...
      if (document.querySelector(".cart-footer") === null) {
        cartDom.insertAdjacentHTML(
          "afterend",
          `
      <div class="d-flex flex-Row shadow-sm card cart-footer mt-2 mb-3 animated flipInX Ccontainer1 ">
        <div class="p-2">
          <button class="btn badge-danger clear-cart-btn" type="button" data-action="clear-cart" onclick="pCart()">Clear Cart
        </div>
        <div class="p-2 ml-auto">
          <button class="btn badge-dark clear-cart-btn" type="button" data-action="check-out">Pay <span class="pay" onclick="pRemove()"></span> 
            &#10137;
        </div>
      </div>`
        );
...
function pRemove() {
  ...
  const totalCount = document.getElementById("pout-count");
